...
def get_figs():
    ...
    fig = px.pie(energy_breakdown, values='Total Consumption', names='Energy Description',
                 title="Business Units' Energy Utilization Breakdown in Calgary")
    ...
    fig2 = px.line(energy_by_year, x='Year', y='Total Consumption', color='Energy Description',
                   title="Business Units' Yearly Energy Utilization in Calgary")
    fig3 = px.line(energy_bd_year_not_elec, x='Year', y='Total Consumption', color='Energy Description',
                   title="Business Units' Energy Utilization Breakdown in Calgary")
    ...
    app = dash.Dash(prevent_initial_callbacks=True)
    app.layout = html.Div(
        [dcc.Location(id="url"), html.Div([dcc.Graph(id="graph1", figure=fig, style={'width': '80vw'})]),
